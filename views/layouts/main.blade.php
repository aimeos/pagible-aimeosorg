...
<html class="no-js" data-theme="light" lang="{{ cms($page, 'lang') }}" dir="{{ in_array(cms($page, 'lang'), ['ar', 'az', 'dv', 'fa', 'he', 'ku', 'ur']) ? 'rtl' : 'ltr' }}">
    <head>
        ...
        <meta name="theme-color" content="{{ cms($page, 'config.theme.data.--pico-contrast') ?: '#103050' }}">
    ...
    <body class="theme-aimeos type-{{ cms($page, 'type') ?: 'page' }}">
        <a href="#main" class="skip-link">{{ __('Skip to main content') }}</a>
        <dialog id="modal-search" class="search">
            <article>
                <header>
                    <form action="{{ route('cms.search', cmsrouteparams(['q' => '_term_'])) }}" toolname="search" tooldescription="{{ __('Search the website and return matching pages with their titles and links') }}" toolautosubmit>
                        ...
                        <a href="{{ cmsroute($nav->ancestors()->first() ?? $page) }}" title="{{ config('app.name') }}" aria-label="{{ config('app.name') }}">
                            ...
                            @foreach($page->ancestorsAndSelf->reverse() as $navItem)
                                @if($fileId = cms($navItem, 'config.logo.data.file.id'))
                                    @if($brandFile = cmsfile($navItem, $fileId))
                                        <img src="{{ cmsasset($navItem, $brandFile) }}" alt="{{ config('app.name') }}">
                                        @break
                                    @endif
                                @endif
                            @endforeach
                            @unless($brandFile)
                                <img src="{{ cmstheme($page, 'logo-aimeos-blue.svg') }}" alt="Aimeos">
                            @endunless
                        </a>
                    </li>
                    <li class="menu-close">
        ...
        @if(!$isLanding && $nav->ancestors()->count() > 1)
        ...
        @if($isLanding)
            <script defer src="{{ cmstheme($page, 'landing.js') }}"></script>
        @endif
